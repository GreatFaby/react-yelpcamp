...
router.get('/campgrounds/:id', (req, res) => {
    ...
    Campground.findById(_id).then((camground) => {
        if (!camground) {
...
router.patch('/campgrounds/:id', auth, async (req, res) => {
    ...
    try {
        const camground = await Campground.findOne({ _id: req.params.id, owner:req.user._id})
...
router.delete('/campgrounds/:id', auth, async (req, res) => {
    try {
        const camground = await Campground.findOneAndDelete({ _id: req.params.id, owner:req.user._id})
